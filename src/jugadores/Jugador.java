...
public void atacarJugador(Jugador enemigo, Monstruo atacante) {
    if (atacante == null) {
        System.out.println("No se seleccionó un monstruo atacante.");
        return;
    }
...
public void jugarMonstruo(Monstruo m, Monstruo sacrificio) {
    ...
    if(!mano.contains(m)){
        ...
        System.out.println(" Ese monstruo no esta en tu mano ");
        return;
    }
    ...
    if (requiereSacrificio(m)) {
        if (sacrificio == null) {
            System.out.println(" Este monstruo requiere sacrificio (Nivel " + m.getNivel() + ")");
            System.out.println(" Selecciona un monstruo de tu campo para sacrificar");
            return;
        }
        if (!campo.contains(sacrificio)) {
            System.out.println(" El monstruo a sacrificar no está en tu campo");
            return;
        }
        // Eliminar el monstruo sacrificado
        campo.remove(sacrificio);
        cementerio.add(sacrificio);
        System.out.println(" ⚔️ " + sacrificio.getNombre() + " ha sido sacrificado!");
    }
...
public void recibirDanio(int danio){
    ...
    System.out.println(nombre + " recibe " + danioReal + " de daño. Vida restante: " + vida);
...
public void jugarTrampa(CartaTrampa carta){
    ...
    trampas.add(carta);
    cartaJugadaEsteTurno = true;
    System.out.println(nombre + " activa la carta trampa: " + carta.getNombre() + " (boca abajo)");
}
...
public void activarTrampas(Jugador oponente, Monstruo atacante) {
    ...
    for (CartaTrampa trampa : trampasActivadas) {
        System.out.println(" ⚠️ Trampa activada: " + trampa.getNombre());
