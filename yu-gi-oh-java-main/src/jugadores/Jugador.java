...
public void atacarJugador(Jugador enemigo, Monstruo atacante, Vista vista) {
    ...
    // Permitir activación de trampas del defensor
    if (!enemigo.getTrampasEnCampo().isEmpty()) {
        int indice = vista.seleccionarTrampaEnCampo(enemigo, "Selecciona una trampa para activar en respuesta al ataque:");
        if (indice >= 0) {
            CartaTrampa trampa = enemigo.getTrampasEnCampo().get(indice);
...
public void atacarJugador(Jugador enemigo, Vista vista) {
    if (campo.isEmpty()) {
    ...
    atacarJugador(enemigo, atacante, vista);
}
